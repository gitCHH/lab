...
def plot(bench, gpu_arch='maxwell'):
    ...
    for percent in ratios:
        m5out_dir = '/home/huan/' + str(2) + 't/' + bench + '/' + gpu_arch + '_m5out/'
    ...
    for percent in ratios:
        m5out_dir = '/home/huan/' + str(4) + 't/' + bench + '/' + gpu_arch + '_m5out/'
        m5out_dir = m5out_dir + bench + percent + '/'
        time_CG = get_PU_runtime(m5out_dir)
        cpu_heights2.append(time_CG[0])
        gpu_heights2.append(time_CG[1])
    for percent in ratios:
        m5out_dir = '/home/huan/' + str(6) + 't/' + bench + '/' + gpu_arch + '_m5out/'
        m5out_dir = m5out_dir + bench + percent + '/'
        time_CG = get_PU_runtime(m5out_dir)
        cpu_heights3.append(time_CG[0])
        gpu_heights3.append(time_CG[1])
    ax = axs
    ax.set_xticklabels(ratios)
    ax.set_xticks(index)
    ax.set_title(bench)
    ax.set_ylabel('CPU Execution Time(ms)')
    ax.set_xlabel('Data Ratio of CPU(%)')
    # ax.set_xlim(0,20)
    # ax.set_ylim(0,10)
    ax.bar(index - bar_width, cpu_heights, bar_width, alpha=opacity, color='r', label='2')
    ...
    ax.bar(index, cpu_heights2, bar_width, alpha=opacity, color='y', label='4')
    # ax.bar(index, gpu_heights, bar_width, alpha=opacity, color='b', label='GPU')
    ax.bar(index + bar_width, cpu_heights3, bar_width, alpha=opacity, color='b', label='6')
    # ax.bar(index, gpu_heights, bar_width, alpha=opacity, color='b', label='GPU')
    ax.legend()
    fig.tight_layout()
    plt.show()
