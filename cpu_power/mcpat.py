...
class McPATEnabledExperiment(Experiment):
    def __init__(self, type, dir, bench=None, l2_size=None, l2_assoc=None,  section_num_to_use=2, gen_mcpat_xml_file=True, mcpat_enabled=True):
        Experiment.__init__(self, type, dir, bench, l2_size, l2_assoc, section_num_to_use)
    ...
    def gen_core(self, tag, i):
        ...
        def gen_l1i():
            with tag('component', id='system.core' + str(i) + '.icache', name='Instruction Cache', type='CacheUnit'):
                ...
                with tag('param', name='size', value='32768'):
                    pass
							   
		with tag('param', name='block_size', value='128'):
                    pass
		with tag('param', name='assoc', value='2'):
             ...
             #            value=self.configs.execute('$.system.cpu[' + str(i) + '].icache.hit_latency')):
                ...
                with tag('param', name='pure_ram', value='0'):
                    pass
		#with tag('param', name='read_accesses', value=str(int(self.stats['system.'+self.cpu_id(i,True)+'.L1Icache.demand_accesses']))):
                #    pass
                #with tag('param', name='read_misses', value=str(int(self.stats['system.'+self.cpu_id(i,True)+'.L1Icache.demand_misses']))):
                #    pass
                #with tag('stat', name='read_accesses',
               #          value=str(
               #              int(self.stats['system.' + self.cpu_id(i, True) + '.L1Dicache.ReadReq_accesses::total']))):
               #     pass
        ...
        with tag('component', id='system.core' + str(i), name='core' + str(i), type='Core'):
            ...
            with tag('param', name='machine_type', value='1'):
                pass
            with tag('param', name='number_hardware_threads', value='1'):
           #          value=self.configs.execute('$.system.cpu[' + str(i) + '].numThreads')):
            ...
            with tag('stat', name='total_instructions', value=str(
                            int(self.stats['system.' + self.cpu_id(i) + '.committedInsts']) )):
                pass
    ...
    def gen_l2(self, tag, i=None):
        with tag('component', id='system.L2' + ('' if i is None else str(i)), name='L2 Cache', type='CacheUnit'):
            with tag('param', name='level', value='2'):
                pass
            with tag('param', name='size', value='262144'):
                pass							   
	    with tag('param', name='block_size', value='128'):
                 pass
	    with tag('param', name='assoc', value='4'):
                 pass
    ...
    if os.path.exists(rootdir):
        for subdir, dirs, files in os.walk(rootdir):
            for dir in dirs:
                ...
                stats_file_name = os.path.join(work_dir, 'stats-last.txt')
